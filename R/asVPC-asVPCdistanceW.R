...
#' @examples
#' asVPC.distanceW(orig.data,sim.data,n.timebin=4, n.sim=100,n.hist=2)
...
{    bintot.N<-n.timebin*n.hist
     ...
     {  if(i<n.hist)
        ...
        temp<-t(apply(temp.quantile,2,function(x) quantile(x,prob=c(alpha/2,0.5,1-alpha/2),na.rm=TRUE)))
