...
#' @examples
#' VPC.graph(orig.data,sim.data,10,50,Y.max=15)
...
{ SIM.CIarea.1 <- NULL
  ...
  findQuantile<-function(DV.data,TIME.data,time.bin.temp,q)
  {
    ...
    {  sel.id<-which(time.bin.temp$COV.bin==time.bin.temp$COV.bin.summary[i,1])
    ...
    colnames(DV.q)[-(1:(ncol(DV.q)-length(q)))]<-paste("Q",round(q*100),"th",sep="")
    return(DV.q)
  }
  ...
  findQuantileCI<-function(sim.data,TIME.data,time.bin.temp,q,alpha,N.sim)
  {
    orig.sim.Q.temp<-apply(sim.data,2,function(x) 
      findQuantile(x,TIME.data,time.bin.temp,q)[,-(1:ncol(time.bin.temp$COV.bin.summary))])
    keep.name<-NULL#names(orig.sim.Q.temp[[1]])
    orig.sim.Q.temp<-array(unlist(orig.sim.Q.temp),dim=c(nrow(time.bin.temp$COV.bin.summary),length(q),N.sim))
    Q.CI<-list()
    for(i in 1:length(q))
    { Q.CI[[i]]<-cbind(findQuantile(sim.data,orig.data$TIME,time.bin.temp,q[i])[,4],
                       t(apply(orig.sim.Q.temp[,i,],1,function(x) quantile(x,probs=c(alpha/2,1/2,1-alpha/2),na.rm=TRUE))))
