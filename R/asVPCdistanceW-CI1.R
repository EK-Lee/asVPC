...
#' data(origdata)
#' data(simdata)
#' asVPC.distanceW.CI(origdata,simdata,N.timebin=10, n.sim=100,n.hist=3)

asVPC.distanceW.CI1<-function(orig.data,sim.data,n.sim,
  ...
  if(is.null(main.title))
    main.title<-paste("DIST: N.timebin=",N.timebin,", n.hist=",n.hist,sep="")
  ...
  for(i in 1:bintot.N){
    ...
    if(bintot.N<length(table(orig.data$TIME))){
      ...
      #      temp.weight<-(max(dist.temp)-dist.temp)/diff(range(orig.data$TIME))
    } else{
  ...
  temp.group<-factor(c(orig.Q[,2]>=Q.CI[[1]][,4] & orig.Q[,2]<=Q.CI[[1]][,6],
                       orig.Q[,3]>=Q.CI[[2]][,4] & orig.Q[,3]<=Q.CI[[2]][,6],
                       orig.Q[,4]>=Q.CI[[3]][,4] & orig.Q[,4]<=Q.CI[[3]][,6]),
